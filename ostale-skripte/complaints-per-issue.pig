...
issues = FOREACH allComplaints GENERATE issue; 

-- group by product name
issuesGroup = GROUP issues BY issue;

-- count how many times each product name appers
issuesCount = FOREACH issuesGroup GENERATE group AS issueName:chararray, COUNT(issues) AS count:long;

-- sort by number of appearance
sortIssuesDesc = ORDER issuesCount BY count DESC;
STORE sortIssuesDesc INTO 'hdfs://namenode:8020/output' using CSVExcelStorage(',');