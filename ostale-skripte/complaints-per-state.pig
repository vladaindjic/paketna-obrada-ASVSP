...
states = FOREACH allComplaints GENERATE state; 

-- group by product name
statesGroup = GROUP states BY state;

-- count how many times each product name appers
statesCount = FOREACH statesGroup GENERATE group AS stateName:chararray, COUNT(states) AS count:long;

-- sort by number of appearance
sortStatesDesc = ORDER statesCount BY count DESC;
STORE sortStatesDesc INTO 'hdfs://namenode:8020/output' using CSVExcelStorage(',');