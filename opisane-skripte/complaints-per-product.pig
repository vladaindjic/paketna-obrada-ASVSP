...
products = FOREACH allComplaints GENERATE product; 

-- group by product name
productsGroup = GROUP products BY product;

-- count how many times each product name appers
productsCount = FOREACH productsGroup GENERATE group AS productName:chararray, COUNT(products) AS count:long;

-- sort by number of appearance
sortProductsDesc = ORDER productsCount BY count DESC;

-- print on terminal
--DUMP sortProductsDesc;

STORE sortProductsDesc INTO 'hdfs://namenode:8020/output' using CSVExcelStorage(',');